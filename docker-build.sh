...
echo 
echo "/######################################################################/"
echo " Build new image terrabrasilis/deter-last-obs-task:${TAG_VERSION} "
echo "/######################################################################/"
echo

docker build $NO_CACHE -t "terrabrasilis/deter-last-obs-task:${TAG_VERSION}" -f env-scripts/Dockerfile .
...
if [[ ! "$SEND_TO_HUB" = "yes" ]]; then
    ...
    docker push "terrabrasilis/deter-last-obs-task:${TAG_VERSION}"
fi