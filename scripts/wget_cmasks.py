...
#define bioma
bioma = os.getenv("TARGET_BIOME", "Amazonia")
assert bioma
...
# database params
host=os.getenv("PGHOST","192.168.15.9")
database=os.getenv("PGDB", "deter_r")
...
sql = f"""
SELECT SUBSTRING(view_date::text,0,8)
FROM {tabledeter}
WHERE view_date >= (SELECT MAX(data) FROM public.last_day_processed)
GROUP BY 1
"""
cur.execute(sql)
missing_year_month = cur.fetchall()

for a_year_month in missing_year_month:
    year_month = a_year_month[0]
    #define mes-ano
    ano_mes = str(year_month).replace('-', '_')
    datai = f"{year_month}-01"
